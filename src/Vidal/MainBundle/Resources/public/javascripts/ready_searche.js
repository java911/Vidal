$(document).ready(function() {
	...
	$searcheForm.find('.search-query')
		.autocomplete({
			...
			source:    function(request, response) {
				$.ajax({
					url:      "http://www.vidal.ru:9200/website/autocompleteext/_search",
					type:     "POST",
					dataType: "JSON",
	...
	$('#searche_nozology')
		.autocomplete({
			...
			source:    function(request, response) {
				$.ajax({
					url:      "http://www.vidal.ru:9200/website/autocomplete_nozology/_search",
					type:     "POST",
					dataType: "JSON",
	...
	$('#searche_contraindication')
		.autocomplete({
			...
			source:    function(request, response) {
				$.ajax({
					url:      "http://www.vidal.ru:9200/website/autocomplete_contraindication/_search",
					type:     "POST",
					dataType: "JSON",
					data:     '{ "query":{"query_string":{"query":"' + request.term + '*"}}, "fields":["name","code"], "size":15, "highlight":{"fields":{"name":{}}} }',
					success:  function(data) {
