...
class DropNonRusCommand extends ContainerAwareCommand
{
	...
	protected function execute(InputInterface $input, OutputInterface $output)
	{
		...
		$output->writeln("... DELETE FROM document WHERE YearEdition > 2015 OR (CountryEditionCode != 'RUS' AND ArticleID NOT IN (1,4))");
		$stmt = $pdo->prepare("DELETE FROM document WHERE YearEdition > 2015 OR (CountryEditionCode != 'RUS' AND ArticleID NOT IN (1,4))");
		$stmt->execute();
